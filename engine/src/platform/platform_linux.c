...
b8 plat_pump_messages(plat_state *state) {
    ...
    while ((event = xcb_poll_for_event(state->connection))) {
        ...
        case XCB_KEY_RELEASE: {
            ...
            // xcb_key_release_event_ t are the same
...
void *plat_alloc(u64 size, b8 aligned) { return malloc(size); }

void plat_free(void *block, b8 aligned) { free(block); }

void *plat_zero_memory(void *block, u64 size) { return memset(block, 0, size); }
...
void plat_console_write(const char *message, u8 color) {
    const char *color_strings[LOG_LEVELS] = {"0;41", "1;31", "1;33",
                                             "1;32", "1;34", "1;30"};
...
b8 plat_create_vulkan_surface(plat_state *state, vulkan_context *context) {
