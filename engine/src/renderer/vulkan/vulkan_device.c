...
b8 vulkan_device_create(vulkan_context *context) {
    ...
    for (u32 i = 0; i < index_count; ++i) {
        ...
        // TODO: Enable this for a future ehancement
...
void vulkan_device_destroy(vulkan_context *context) {
    ...
    // Physical deices are not destroyed.
    PINFO("Releasing physical device resources...");
    context->device.physical_device = 0;

    if (context->device.swapchain_support.formats) {
        // TODO: change to kfree
        pfree(context->device.swapchain_support.formats,
    ...
    if (context->device.swapchain_support.present_modes) {
        pfree(context->device.swapchain_support.present_modes,
    ...
    pmemzero(&context->device.swapchain_support.capabilities,
...
void vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (!out_support_info->formats) {
            out_support_info->formats =
                palloc(sizeof(VkSurfaceKHR) * out_support_info->format_count,
                       MEMORY_TAG_RENDERER);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
    ...
    if (out_support_info->present_mode_count != 0) {
        out_support_info->present_modes = palloc(
...
b8 select_physical_device(vulkan_context *context) {
    ...
    if (physical_device_count == 0) {
        ...
        PFATAL("Check out https://vulkan.gpuinfo.org/");
        return FALSE;
    }
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        // TODO: These requirments should probably driven by engine
        // configuration
        vulkan_physical_device_requirements requirments = {};
        requirments.graphics = TRUE;
        requirments.present = TRUE;
        requirments.transfer = TRUE;
        // NOTE: Enable this if compute will be required
        // requirements.compute = TRUE
        requirments.sampler_anisotropy = TRUE;
        requirments.discrete_gpu = FALSE;
        requirments.device_extension_names = darray_create(const char *);
        darray_push(requirments.device_extension_names,
        ...
        b8 result = physical_device_meets_requirements(
            ...
            &requirments, &queue_info, &context->device.swapchain_support);
    ...
    if (!context->device.physical_device) {
        PERROR("No physical device were found which meet the requirements!");
        return FALSE;
    }
...
b8 physical_device_meets_requirements(
    ...
    if (requirements->discrete_gpu) {
        if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
            PINFO("Device is not a discrete GPU, and one is required. "
                  "Skipping.");
            return FALSE;
        }
    }
    ...
    if ((!requirements->graphics ||
        ...
        PTRACE("Graphics Family Index: %i",
               out_queue_info->graphics_family_index);
        PTRACE("Present Family Index:  %i",
               out_queue_info->present_family_index);
        PTRACE("Transfer Family Index: %i",
               out_queue_info->transfer_family_index);
        PTRACE("Compute Family Index:  %i",
        ...
        if (out_swapchain_support->format_count < 1 ||
            ...
            if (out_swapchain_support->formats) {
                pfree(out_swapchain_support->formats,
                      sizeof(VkSurfaceFormatKHR) *
                          out_swapchain_support->format_count,
                      MEMORY_TAG_RENDERER);
            }
            if (out_swapchain_support->present_modes) {
                pfree(out_swapchain_support->present_modes,
                      sizeof(VkPresentModeKHR) *
                          out_swapchain_support->present_mode_count,
                      MEMORY_TAG_RENDERER);
            }
            PINFO("Required swapchain support not present, skiping device.");
            return FALSE;
        }
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                available_extensions = palloc(sizeof(VkExtensionProperties) *
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    for (u32 j = 0; j < available_extension_count; ++j) {
                        if (str_equals(requirements->device_extension_names[i],
                    ...
                    if (!found) {
                        PINFO("Required extension not found: '%s', skipping "
                              "device.",
                              requirements->device_extension_names[i]);
                        pfree(available_extensions,
        ...
        if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
            PINFO("Device does not support samplerAnistropy, skipping.");
            return FALSE;
        }
