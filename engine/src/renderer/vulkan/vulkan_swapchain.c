...
void vulkan_swapchain_present(vulkan_context *context,
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        // out of date, trugger recreation
        vulkan_swapchain_recreate(context, context->framebuffer_width,
                                  context->framebuffer_height, swapchain);
    } else if (result != VK_SUCCESS) {
        PFATAL("Failed to present swap chain image!");
    }
}
...
void create(vulkan_context *context,
    ...
    // Clap to the value allowed by the GPU
    VkExtent2D min =
    ...
    swapchain_extent.width =
        PCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height =
        PCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device,
                                     ...
                                     0));
    if (!swapchain->images) {
        swapchain->images = (VkImage *)palloc(
            sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView *)palloc(
    ...
    if (!vulkan_device_detect_depth_format(&context->device)) {
        ...
        PFATAL("Failed to find a supported format!");
    }
